...
### OpenCode V1/V2 — Dual Version

Pantheon's shared config is **V1-shaped and valid under both OpenCode versions**:

| | V1 | V2 (beta) |
|---|---|---|
| Binary | `opencode` (1.18.18) | `opencode2` (v0.0.0-next-17444) |
| Config locations | `~/.config/opencode/`, `<project>/opencode.json`, `.opencode/` | Same — V2 reads the same locations |
| Config handling | Loads V1 shape directly | Normalizes V1 fields **in memory** — no rewrite, no native V2 conversion |
...
- Instruction content consolidated into `AGENTS.md` (generated by `scripts/build-agents-md.mjs` from `src/instructions/*.instructions.md`) — V2 accepts but does **not** load the `instructions` config key; both versions auto-load `AGENTS.md`.
- `npx pantheon-opencode init --version v2` labels an install for V2 (informational — the config is shared).

**Known V2 beta gaps:**

- `instructions` config key accepted but not loaded → content lives in `AGENTS.md`.
- `formatter` / `lsp` config not executed.
- `--subtask` delegation flag is a no-op.

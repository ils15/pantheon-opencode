...
const KNOWN_AGENTS_ORDER = ['zeus','athena','themis','hermes','aphrodite','demeter','prometheus','hephaestus','apollo','nyx','gaia','iris','mnemosyne','talos']
...
function visionLabel(def){
  ...
  let visionFlag = '—'
  try{ visionFlag = hasVision(m) ? '👁️ vision' : 'text-only' }catch{ visionFlag = 'unknown' }
...
function capabilityBadge(model){
  try{
    ...
    const entry = CAPABILITY_TABLE.find(e => model.startsWith(e.prefix) || model.slice(model.lastIndexOf('/')+1).startsWith(e.prefix) || model.slice(model.lastIndexOf('/')+1)===e.prefix)
...
function buildSummaryTable(){
  const lines=[]
  lines.push('| Preset | Provider / BaseURL | Key env (nome) | Pricing 2026 | Vision (fallback) | Papéis / Modelos |')
  lines.push('|---|---|---|---|---|---|')
  for(const name of Object.keys(presets)){
    ...
    const papelSummary = Object.entries(groups).map(([model,list])=>`${list.join(',')}: ${model}`).join('<br>')
...
function buildMatrixTable(){
  ...
  const lines=[]
  // header
  lines.push(`| Agente | ${presetNames.map(n=>`\`${n}\``).join(' | ')} | Capability (vision / maxEffort) |`)
  lines.push(`|---|${presetNames.map(()=> '---').join('|')}|---|`)
  // collect all agents
  const allAgents = new Set()
  for(const def of Object.values(presets)) for(const a of Object.keys(def.agents ?? {})) allAgents.add(a)
...
function buildDetailedMatrixWithEffort(){
  ...
  for(const agent of sorted){
    const cells = presetNames.map(presetName=>{
      ...
      if(!spec) return '—'
      let vision = '—'
      try{ vision = hasVision(spec.model) ? '👁️' : '·' }catch{ vision='?' }
...
function buildPricingTable(){
  ...
  for(const name of Object.keys(presets)){
    ...
    const costHint = name==='go-free' ? '$0 (free-tier Zen, quota limitada)' : name==='go-fast' ? 'Baixo custo (Go gateway, baixa latência)' : name==='go-premium' ? 'Premium (Go gateway, melhor qualidade)' : 'Pago por uso (OpenAI direto)'
...
function buildCapabilityTable(){
  ...
  for(const e of CAPABILITY_TABLE){
    lines.push(`| \`${e.prefix}\` | ${e.maxEffort ?? '—'} | ${e.vision ? '👁️' : '—'} | ${e.stripEffort ? 'yes' : 'no'} |`)
  }
  return lines.join('\n')
}
...
// default: print all
console.log('<!-- Generated from src/routing.yml + src/pantheon/presets.mjs + scripts/install/model-picker.mjs — do not edit manually. Run: node scripts/generate-preset-docs.mjs -->')
console.log('')

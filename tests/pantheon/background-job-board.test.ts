...
await testAsync('registerLaunch copies contextFiles', async () => {
  ...
  const job = await board.registerLaunch(
    makeLaunch({
      contextFiles: [
        { path: '/foo/bar.ts', lineCount: 42, lastReadAt: 1000 },
      ],
    }),
  )
...
await testAsync('updateStatus completed → completed is idempotent', async () => {
  ...
  const updated = await board.updateStatus({ taskID: job.taskID, state: 'completed', resultSummary: 'v2' })
...
await testAsync('markCancelled from non-running terminal throws', async () => {
  ...
  await assert.rejects(
    board.markCancelled(job.taskID),
    /Cannot cancel/,
  )
})
...
await testAsync('formatForPrompt returns formatted jobs', async () => {
  const board = new BackgroundJobBoard()
  await board.registerLaunch(makeLaunch({ parentSessionID: 's1', agent: 'apollo', description: 'Search files' }))
  const j2 = await board.registerLaunch(makeLaunch({ parentSessionID: 's1', agent: 'hermes', description: 'Implement endpoint' }))
...
await testAsync('pruneExpired removes old jobs', async () => {
  const board = new BackgroundJobBoard()
  await board.registerLaunch(makeLaunch({ taskID: 'old-job' }))

  // Simulate age by directly manipulating the record
  const job = board.get('old-job')!
  job.updatedAt = Date.now() - 100_000

  await board.registerLaunch(makeLaunch({ taskID: 'new-job' }))
  const newJob = board.get('new-job')!
  newJob.updatedAt = Date.now() - 1_000

  board.pruneExpired(10_000) // TTL = 10s

  assert.equal(board.get('old-job'), undefined)
...
await testAsync('all agent prefixes are correct', async () => {
  ...
  for (const [agent, expectedPrefix] of agents) {
    const job = await board.registerLaunch(
      makeLaunch({ agent, parentSessionID: 'ses_prefix' }),
    )
    assert.ok(job.alias.startsWith(expectedPrefix), `${agent} → ${job.alias} expected prefix ${expectedPrefix}`)
  }
})

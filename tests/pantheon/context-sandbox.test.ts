...
async function main() {
  ...
  await testAsync("sandboxWebfetch: over limit (6000) — truncated head 2000 + marker", async () => {
    ...
    assert.ok(out.length < input.length, "output must be shorter than input? actually head+marker vs 6000: 2000+~80 <6000 true")
  })
  ...
  await testAsync("resolveSandboxConfig: partial limits override", async () => {
    const cfg = resolveSandboxConfig({ limits: { read: { maxLines: 10, keepHead: 3, keepTail: 1 } } })
  ...
  await testAsync("resolveSandboxConfig: invalid limits → fall back", async () => {
    const cfg = resolveSandboxConfig({ limits: { read: { maxLines: "bad" } } } as unknown as Record<string, unknown>)

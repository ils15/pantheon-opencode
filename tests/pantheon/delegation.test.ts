...
async function main() {
  ...
  await testAsync(
    '(a2) legacy read: oversized report is capped head+tail with marker + report path (native parity)',
    async () => {
      const tmp = mkdtempSync(join(tmpdir(), 'delegation-read-cap-'))
      try {
        const board = new BackgroundJobBoard()
        const client = new FakeClient()
        client.messagesResult = [
        ...
        const tools = createDelegationTools({
          board,
          client,
          options: { rootSessions: new Set([ROOT]), outputDir: tmp, readMaxChars: 900 },
        })
        ...
        const content = await readPromise
        assert.ok(content.length < 1600, `capped result stays small, got ${content.length}`)
        assert.ok(content.includes('HEAD HEAD'), 'head kept')
        assert.ok(content.includes('TAILTAILTAIL'), 'tail kept — the conclusion survives')
        assert.ok(!content.includes('MIDMIDMID'), 'middle dropped')
        assert.ok(/\[TRUNCATED: \d+ of \d+ chars hidden/.test(content), 'explicit marker')
        assert.ok(content.includes('apo-1.md'), 'marker points at the full report file')
        assert.ok(
          content.includes('## Agent Activity'),
          'activity section still appended after the cap',
        )
      } finally {
        rmSync(tmp, { recursive: true, force: true })
      }
